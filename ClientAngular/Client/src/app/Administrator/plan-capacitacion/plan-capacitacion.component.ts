...
import {PlanCapacitacion} from '../../models/plan-capacitacion';
...
export class PlanCapacitacionComponent implements OnInit {

  public actual:PlanCapacitacion;
  public selected:PlanCapacitacion;
  public listado:any;
  ...
  constructor(private _route:ActivatedRoute,
    ...
    this.actual = new PlanCapacitacion(this.any, this.any, '',  []);
    this.listado = [];
    this.getAll();
  }
  ...
  getAll() {
    this._capacitacionService.getAll().subscribe(response => {
  ...
  save() {
    ...
    if(this.categoria && this.listadoPara.lenght!=0){
      if (this.newObj) {
        ...
        this._capacitacionService.save(this.actual).subscribe(
                response => {
              if (response) {
                this.status = "success";
                this.actual = new PlanCapacitacion(this.any, this.any, '',  []);
        ...
        //funcion update
        this.actual.parametros = this.listadoPara;
        this._capacitacionService.update(this.actual).subscribe(
                response => {
              if (response) {
                this.status = "success";
                this.getAll()
  ...
  cloneObj(obj:PlanCapacitacion):PlanCapacitacion {
...
class PrimeObj implements PlanCapacitacion {
