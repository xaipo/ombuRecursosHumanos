...
import {AplicacionCapacitacionService} from '../../services/aplicacion-capacitacion.service';
import {PlanCapacitacionService} from '../../services/plan-capacitacion.service';
import {EmpleadoService} from '../../services/empleado.service';
import {AplicacionCapacitacion} from '../../models/aplicacion-capacitacion';
import {dateFormatPipe} from '../../pipes/datePipe';

@Component({
  selector: 'app-apicacion-capacitacion',
  templateUrl: './apicacion-capacitacion.component.html',
  styleUrls: ['./apicacion-capacitacion.component.css'],
  providers: [
    AplicacionCapacitacionService,
    PlanCapacitacionService,
    EmpleadoService
  ]
})
export class ApicacionCapacitacionComponent implements OnInit {

  public actual:AplicacionCapacitacion;
  public selected:AplicacionCapacitacion;
  public listado:any;
  ...
  constructor(private _route:ActivatedRoute,
             ...
             private _aplicacionService:AplicacionCapacitacionService,
             private _empleadoService:EmpleadoService,
             private _capacitacionService:PlanCapacitacionService) {
    this.actual = new AplicacionCapacitacion(this.any, this.any, this.any,this.any,[]);
    this.listado = [];
    this.getAll();
  }
  ...
  save() {
    ...
    if(this.capacitacion && this.empleado && this.listadoPara.lenght!=0){
      if (this.newObj) {
        // funcion save
        this.actual.parametros = this.listadoPara;
        this.actual.capacitacion = this.capacitacion._id;
  ...
  onRowSelect(event) {
    ...
    this.capacitacion = this.actual.capacitacion;
    this.empleado = this.actual.empleado;
  }

  cloneObj(obj:AplicacionCapacitacion):AplicacionCapacitacion {
  ...
  filterSingle(event) {
    ...
    this._capacitacionService.getAll().subscribe(objs => {
      //var auxObjs = objs;
  ...
  filter(query, objs: any[]):any[] {
    ...
    for(let i = 0; i < objs.length; i++) {
      let capacitacion = objs[i];
      if(capacitacion.descripcion.toLowerCase().indexOf(query.toLowerCase()) == 0) {
        filtered.push(capacitacion);
      }
    }
    return filtered;
  }
...
// FUNCIONES para el AUTOCOMPLETAR  EMPLEADO **********************************************************************************
  ...
  filterSingleParam(event) {
    this.filteredParam = [];
    for(let i = 0; i < this.capacitacion.parametros.length; i++) {
      let obj = this.capacitacion.parametros[i];
...
class PrimeObj implements AplicacionCapacitacion {

  constructor(public _id, public capacitacion, public empleado, public fecha, public parametros) {
  }
}
