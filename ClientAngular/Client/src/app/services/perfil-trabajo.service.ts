...
export class PerfilTrabajoService {
  ...
  save(obj_to_register){
    ...
    return this._http.post(this.url+'/perfilTrabajo', params, {headers: headers})
      .map(res=>res.json());
  }
  ...
  update(obj_to_update){
    ...
    return this._http.put(this.url+'/perfilTrabajo/'+obj_to_update._id, params, {headers: headers})
      .map(res=>res.json());
  }
}
